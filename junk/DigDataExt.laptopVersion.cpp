...
DigDataExt::DigDataExt(const string setup_file_name, const string dig_file_name, const string out_file_name): SetupFileName(setup_file_name), DigFileName(dig_file_name), OutFileName(out_file_name)
{
  ...
  for (int ch=0; ch<PMTNum+ MAX_X742_GROUP_SIZE;ch++)
	...
	gwf_sample[cn][ch]=new TGraph(); gwf_sample[cn][ch]->SetTitle(Form("WF sample %d - %s",cn,PMTTitle[ch].c_str()));      
      ...
      h2_ch_v[ch]=new TH2F(Form("ch%d_2d",ch),Form("ADC distribution ch%d by time; TimeSince event 0 [usec]; Count [mVolts]",ch),nbins,T_first_usec-T_first_usec,T_last_usec-T_first_usec,1e5,0,3000*3000);
  ...
  DigDataFile.close();
  OutDataFile.close();
  //file  TFile* myfile = TFile::Open("file.root","recreate");
  ...
  drawObjects((TObject**) h2_ch_v, "power_2d_a", "Overall power by channel, over time",run_time_stamp,0,0,1);
  ...
  drawObjects((TObject**)  h2_ch_wf_bins, "ch_wf_bin_2d", "Bins distribution  over time",run_time_stamp, 0,0,1);

  drawObjects((TObject**)  h_ch_wf_bins, "ch_wf_bin_1d", "Overall Bins distributions",run_time_stamp, 0,0,1);		

  drawObjects((TObject**) h_ch_v,"voltages","Voltages distribution baseline removed",run_time_stamp,1,1,0);

  drawObjects((TObject**) h_ch_v_w_base,"voltages_w_base","Voltages distribution w baseline",run_time_stamp,1,1,0);

  for (int cn=0; cn<Nsamples; cn++)
    drawObjects((TObject**) gwf_sample[cn],Form("WF_sample_%d",cn),wf_title[cn],run_time_stamp,0,0,0);

  drawObjects((TObject**) h_ch_sigma,"sigmas","Number of over sigma events", run_time_stamp,1,0,0);

  drawObjects((TObject**) gpower2_dt_ch,"power2","power square", run_time_stamp,1,0,0);
  ...
  draw2Objects((TObject**)	h1_baseline_i_ch, (TObject**) h1_baseline_f_ch,"h_baseline_i","Direxeno Baseline ",run_time_stamp,0,0,0);
...
void drawObjects(TObject** objects, TString name, TString title,  TString timestring, int logy, int fits, int logz)
{
...
void draw2Objects(TObject** objects1, TObject** objects2,TString name, TString title, TString timestring,int logy, int fits, int logz)
{

...
static void PeakCorrection(CAEN_DGTZ_X742_GROUP_t *dataout) {
	...
	int length = sizeof(dataout->DataChannel) / sizeof(dataout->DataChannel[0]);
	if (8+chaux_en!=length){
        printf("problem lebgth chaux = %d, %d \n",8+chaux_en,length );
	return;
       }
...
void ApplyDataCorrection(CAEN_DGTZ_DRS4Correction_t* CTable, CAEN_DGTZ_DRS4Frequency_t frequency, int CorrectionLevelMask, CAEN_DGTZ_X742_GROUP_t *data) {
	...
	if (!timeCorrection) {
		...
		for(j=1; j < 1024; j++) {
			...
			int length = sizeof(data->DataChannel) / sizeof(data->DataChannel[0]);
			if (8+trg!=length){
			printf("problem 8+trg length = %d, %d \n",8+trg,length );
			return;
		}

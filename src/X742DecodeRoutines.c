...
//static int V1742UnpackEventGroup(uint32_t group, uint32_t *datain, CAEN_DGTZ_X742_GROUP_t *dataout,float &Tsamp) {
static int V1742UnpackEventGroup(uint32_t group, uint32_t *datain, CAEN_DGTZ_X742_GROUP_t *dataout,float &Tsamp, uint32_t &freq) {
...
//    uint32_t freq;
   ...
   freq = (datain[0] >> 16 ) & 0x3;
	...
	//		printf ("this time tag = %ld \n ",datain[size1+size2+1] & 0x3FFFFFFF);
	...
	do {
		...
		if (counter == numEvent) {
			if ((i + (uint32_t) evtSize) < buffsize) {
				*EventPtr =(buffer+i);
				// ---- PRINT EVENT INFO ----
				uint32_t *header = (uint32_t *)(*EventPtr);

				uint32_t rawSize     = header[0];
				uint32_t boardWord   = header[1];
				uint32_t eventCount  = header[2];
				uint32_t triggerTime = header[3];

				uint8_t boardID = (boardWord >> 27) & 0x1F; // bits [27:31], mask = 0xF8000000 >> 27
				if (counter<-1 ) {
				  printf("evtsize: %d \t header size: %d \t counter: %d \t i: %u\t Event #%d:\t",evtSize,EVENT_HEADER_SIZE,counter, i,numEvent);
				  printf("  Board ID: %u\t", boardID);
				  printf("  Event Counter: %u\t", eventCount);
...
int32_t X742_DecodeEvent(char *evtPtr, void **Evt) {
  //	CAEN_DGTZ_X742_EVENT_t *Event;
	...
	uint32_t timeTag=  *(long *)(evtPtr+12) & 0xFFFFFFFF;
	int counter = *(long *)(evtPtr+8) & 0x0000FFFF;
	//printf ("%d %u time tag %ld %d\n",timeTag,timeTag,counter,sizeof(long));
...
//	Event = (CAEN_DGTZ_X742_EVENT_t *) malloc(sizeof(CAEN_DGTZ_X742_EVENT_t));
	Event = (XeEvent *) malloc(sizeof(XeEvent));

	if (Event == NULL) return -1;
	memset( Event, 0, sizeof(XeEvent));
	...
	for (g=0; g<X742_MAX_GROUPS; g++) {
	  Event->groupTtag[g]=0;
		...
		for (j=0; j<MAX_X742_CHANNEL_SIZE; j++) {
			Event->DataGroup[g].DataChannel[j]= (float*) malloc(X742_FIXED_SIZE * sizeof (float));
//			Event->DataGroup[g].DataChannel[j]=  malloc(X742_FIXED_SIZE * sizeof (float));
		...
		float Tsamp;
		uint32_t freq;
		//printf ("while decoding group %d val %d \n",g,Event->groupTtag[g]);
		size=V1742UnpackEventGroup(g,pbuffer,&(Event->DataGroup[g]), Tsamp, freq);

		Event->Freq=freq;
		Event->Tstep=Tsamp;
		Event->T0 = timeTag;;
		//	printf ("this time tag = %ld %d \n ",timeTag,timeTag);
		Event->eventNumber=counter;
		//  long int groupTtag[4];
		Event->groupTtag[g]=Event->DataGroup[g].TriggerTimeTag;
		//	printf ("while decoding group %d val %d \n",g,Event->groupTtag[g]);
